...
export type CameraClass = "K0" | "K1" | "K2" | "K3" | "K4" | "N";
...
export interface UserAnswers {
  ...
  /** Kui true, jäävad alles low-light, N/starlight või thermal öövaatluseks sobivad droonid. */
  nightVisionRequired?: boolean;

  /** Kui true, saab zoom sobivusskooris lisakaalu. */
  zoomPreferred?: boolean;

  /** Kui true, lubatakse ka professionaalsemad / enterprise-klassi mudelid. */
...
const CAMERA_ORDER: CameraClass[] = ["K0", "K1", "K2", "K3", "K4", "N"];
...
const SCORE = {
  ...
  thermalPreferred: 12,
  nightVisionMatch: 18,
  professionalAllowedMatch: 6,
  fpvTypeMatch: 20,
  obstacleAvoidancePreferred: 8,
  penaltyMissingThermalWhenRequired: -30,
  penaltyMissingNightCapabilityWhenRequired: -30,
...
const USE_CASE_REQUIREMENTS: Record<UseCaseId, Partial<{
  ...
  night_observation: { cameraMin: "K2", nightVisionRequired: true, thermalPreferred: true, zoomPreferred: true, preferredWind: "T2" },
...
function hasNightCapability(drone: Drone): boolean {
...
function isAtOrAboveCamera(droneClass: CameraClass | undefined, minClass: CameraClass | undefined, drone?: Drone): boolean {
...
function isConsumerScope(drone: Drone): boolean {
  const scope = drone.scope ?? "consumer";
  return ["consumer", "consumer_fpv", "prosumer"].includes(scope);
}

function isProfessionalScope(drone: Drone): boolean {
  return !isConsumerScope(drone);
}

function mergeDerivedRequirements(answers: UserAnswers): Required<Pick<UserAnswers, "thermalRequired" | "nightVisionRequired" | "zoomPreferred">> & {
  minCameraClass: CameraClass;
  minWindClass?: WindClass;
  ...
  for (const useCase of answers.useCases) {
    ...
    if (req.nightVisionRequired) nightVisionRequired = true;
  ...
  return { thermalRequired, nightVisionRequired, zoomPreferred, minCameraClass, minWindClass, preferredFpvTypes, needsBeginnerFriendly, needsTravelFriendly };
}
...
function hardFilter(drone: Drone, answers: UserAnswers, derived: ReturnType<typeof mergeDerivedRequirements>): string[] {
  ...
  // Professionaalsemaid droone ei välistata absoluutselt. Need lubatakse, kui
  // kasutaja valib eriotstarbelise vajaduse või teeb opt-in valiku.
...
function scoreDrone(drone: Drone, answers: UserAnswers, derived: ReturnType<typeof mergeDerivedRequirements>): Omit<Recommendation, "drone" | "group" | "scorePercent" | "excludedReasons"> {
  ...
  if (derived.nightVisionRequired) {
    if (hasNightCapability(drone)) {
      score += SCORE.nightVisionMatch;
      reasons.push("öö-/hämaras vaatlusvõime on kaetud");
    } else {
      score += SCORE.penaltyMissingNightCapabilityWhenRequired;
  ...
  if (answers.professionalAllowed && isProfessionalScope(drone)) {
    score += SCORE.professionalAllowedMatch;
...
export function recommendDrones(catalog: DroneCatalog, answers: UserAnswers, limit = 5): Recommendation[] {
  ...
  const recommendations = catalog.drones.map((drone) => {
    ...
    const maxReasonableScore = 190; // positiivsete punktide ligikaudne lagi koos FPV/thermal lisadega
